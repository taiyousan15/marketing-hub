...
export async function POST(request: NextRequest) {
  try {
    ...
    if (model.provider === "google") {
      ...
      if (!geminiApiKey) {
        return NextResponse.json(
          { error: "GEMINI_API_KEY が設定されていません。.env に GEMINI_API_KEY を追加してください。" },
          { status: 503 }
        );
      }

      const response = await fetch(
        `https://generativelanguage.googleapis.com/v1beta/models/${model.apiId}:generateImages?key=${geminiApiKey}`,
        {
          method: "POST",
      ...
      if (!response.ok) {
        const err = await response.text();
        throw new Error(`Imagen 3 エラー: ${response.status} - ${err}`);
      }

      const data = await response.json();
      const base64Data = data.generatedImages?.[0]?.image?.imageBytes;
      if (!base64Data) throw new Error("レスポンスに画像データが含まれていません");

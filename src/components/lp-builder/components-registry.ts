...
export const COMPONENT_REGISTRY: Record<ComponentCategory, LPComponent[]> = {
  ...
  video: [
    ...
    },
    {
      ...
      category: 'video',
      name: 'インタラクティブ動画',
      description: '選択肢付き動画（分岐・ホットスポット）',
      icon: 'MousePointerClick',
      defaultProps: [
        { key: 'videoUrl', label: '動画URL', type: 'text', value: '', placeholder: 'YouTube/Vimeo/MP4 URL' },
        { key: 'title', label: 'タイトル', type: 'text', value: '', placeholder: '動画タイトル' },
        { key: 'choices', label: '選択肢（ラベル|URL|表示秒数で改行）', type: 'textarea', value: '詳しく見る|https://example.com|30\n申し込む|https://example.com/apply|60', placeholder: 'ラベル|URL|表示秒数（改行で追加）' },
        { key: 'choicePosition', label: '選択肢の位置', type: 'select', value: 'bottom', options: [
          { label: '動画下', value: 'bottom' },
          { label: '動画内オーバーレイ', value: 'overlay' },
        ]},
        { key: 'choiceStyle', label: '選択肢スタイル', type: 'select', value: 'card', options: [
    ...
    },
    {
      ...
      category: 'video',
      name: '時限表示CTA付き動画',
      description: '動画視聴後に時間経過でボタンが表示される',
      icon: 'TimerReset',
      defaultProps: [
        { key: 'videoUrl', label: '動画URL', type: 'text', value: '', placeholder: 'YouTube/Vimeo/MP4 URL' },
        { key: 'title', label: '動画タイトル', type: 'text', value: '', placeholder: '動画タイトル' },
        { key: 'ctaText', label: 'CTAボタンテキスト', type: 'text', value: '今すぐ申し込む', placeholder: 'ボタンテキスト' },
        { key: 'ctaUrl', label: 'CTA遷移先URL', type: 'text', value: '', placeholder: 'https://' },
        { key: 'ctaSubText', label: 'CTAサブテキスト', type: 'text', value: '', placeholder: 'ボタン下のテキスト' },
        { key: 'delaySeconds', label: '表示までの秒数', type: 'number', value: 30 },
        { key: 'triggerType', label: 'トリガー', type: 'select', value: 'elapsed', options: [
          { label: 'ページ表示から経過', value: 'elapsed' },
          { label: '動画再生開始から経過', value: 'play' },
          { label: '動画視聴完了後', value: 'ended' },
        ]},
        { key: 'hideAtSeconds', label: '非表示秒数（0=常時表示）', type: 'number', value: 0 },
        { key: 'ctaColor', label: 'ボタン色', type: 'color', value: '#dc2626' },
        { key: 'ctaTextColor', label: 'ボタン文字色', type: 'color', value: '#ffffff' },
        { key: 'animation', label: 'アニメーション', type: 'select', value: 'fadeIn', options: [
          { label: 'フェードイン', value: 'fadeIn' },
          { label: 'スライドアップ', value: 'slideUp' },

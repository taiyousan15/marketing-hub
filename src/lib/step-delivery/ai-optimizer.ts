...
async function getEngagementHistory(
  ...
  for (const channel of channels) {
    // MessageHistory only supports EMAIL, LINE, SMS (not WHATSAPP)
    if (channel === 'WHATSAPP') {
      // TODO: Add WHATSAPP support to MessageHistory or query WhatsApp logs separately
      continue;
    }
...
async function predictOptimalHour(
  ...
  // MessageHistory only supports EMAIL, LINE, SMS (not WHATSAPP)
  if (channel === 'WHATSAPP') {
    return 12; // Default to noon for WHATSAPP
  }
...
export async function getChannelStats(
  ...
  for (const channel of channels) {
    // MessageHistory only supports EMAIL, LINE, SMS (not WHATSAPP)
    if (channel === 'WHATSAPP') {
      // TODO: Query WhatsApp logs separately when available
      continue;
    }

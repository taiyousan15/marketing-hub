...
export interface TaiyoHeadlineResult {
  headlines: Array<{ text: string; type: string }>;
  recommended: number;
}

export interface TaiyoBodyResult {
  empathy: string;
  problem: string;
  solution: string;
  benefits: string;
...
export interface TaiyoBulletResult {
  bullets: Array<{ text: string; emotion: string }>;
}

export interface TaiyoCTAResult {
  buttonText: string;
  subText: string;
  urgencyText: string;
  reassurance: string;
...
export interface TaiyoPSResult {
  ps: Array<{ type: string; text: string }>;
}

/**
 * 太陽スタイル ヘッドライン生成（Ollama）
 */
export async function generateTaiyoHeadline(
  config: TaiyoStyleConfig,
  options: LLMGenerateOptions = {}
): Promise<TaiyoHeadlineResult> {
  const { model = defaultModel, temperature = 0.8 } = options;
  const prompt = generateHeadlinePrompt(config);
  ...
  try {
    ...
    const response = await client.chat({
      model,
      messages: [
        { role: "system", content: LP_GENERATION_SYSTEM_PROMPT },
        { role: "user", content: prompt },
      ],
      options: { temperature },
    });
    ...
    return JSON.parse(jsonMatch[0]) as TaiyoHeadlineResult;
  } catch (error) {
    console.error("Failed to generate headline:", error);
    // フォールバック
    return {
      headlines: [
        { text: `${config.productName}で${config.benefits[0] || '理想の結果'}を手に入れる`, type: "ベネフィット型" },
        { text: `なぜ${config.targetAudience}は${config.productName}を選ぶのか`, type: "好奇心型" },
        { text: `${config.painPoints[0] || 'その悩み'}から解放される方法`, type: "問題提起型" },
      ],
      recommended: 0,
    };
  }
...
/**
 * 太陽スタイル ボディコピー生成（Ollama）
 */
export async function generateTaiyoBody(
  config: TaiyoStyleConfig,
  options: LLMGenerateOptions = {}
): Promise<TaiyoBodyResult> {
  const { model = defaultModel, temperature = 0.7 } = options;
  const prompt = generateBodyPrompt(config);
  ...
  try {
    ...
    return JSON.parse(jsonMatch[0]) as TaiyoBodyResult;
  } catch (error) {
    console.error("Failed to generate body:", error);
    // フォールバック
    return {
      empathy: `${config.painPoints[0] || 'その悩み'}で困っていませんか？あなたの気持ち、よくわかります。`,
      problem: `このまま放置すると、状況はさらに悪化するかもしれません。`,
      solution: `${config.productName}なら、その問題を解決できます。`,
      benefits: config.benefits.map(b => `・${b}`).join('\n'),
      proof: `多くの方が既に成果を出しています。`,
      cta: `今すぐ行動を起こしましょう。`,
    };
  }
}
...
/**
 * 太陽スタイル ブレット生成（Ollama）
 */
export async function generateTaiyoBullets(
  config: TaiyoStyleConfig,
  options: LLMGenerateOptions = {}
): Promise<TaiyoBulletResult> {
  const { model = defaultModel, temperature = 0.8 } = options;
  const prompt = generateBulletPrompt(config);
  ...
  try {
    ...
    return JSON.parse(jsonMatch[0]) as TaiyoBulletResult;
  } catch (error) {
    console.error("Failed to generate bullets:", error);
    // フォールバック
    return {
      bullets: config.benefits.map((b, i) => ({
        text: `たった一歩踏み出すだけで、${b}が手に入る`,
        emotion: i % 2 === 0 ? "期待感" : "安心感",
      })),
    };
  }
}
 ...
 * 太陽スタイル CTA生成（Ollama）
 */
export async function generateTaiyoCTA(
  config: TaiyoStyleConfig,
  options: LLMGenerateOptions = {}
): Promise<TaiyoCTAResult> {
  const { model = defaultModel, temperature = 0.7 } = options;
  const prompt = generateCTAPrompt(config);
  ...
  try {
    ...
    return JSON.parse(jsonMatch[0]) as TaiyoCTAResult;
  } catch (error) {
 ...
 * 太陽スタイル 追伸生成（Ollama）
 */
export async function generateTaiyoPS(
  config: TaiyoStyleConfig,
  options: LLMGenerateOptions = {}
): Promise<TaiyoPSResult> {
  const { model = defaultModel, temperature = 0.7 } = options;
  const prompt = generatePSPrompt(config);
  ...
  try {
    ...
    return JSON.parse(jsonMatch[0]) as TaiyoPSResult;
  } catch (error) {
    console.error("Failed to generate PS:", error);
    // フォールバック
    return {
      ps: [
        { type: "特典再確認", text: `P.S. 今なら特別特典もお付けします。` },
        { type: "緊急性", text: `P.P.S. この価格でのご提供は今回限りです。` },
      ],
    };
  }
}
...
/**
 * 太陽スタイル 全コピー一括生成（Ollama）
 */
export async function generateTaiyoFullCopy(
  config: TaiyoStyleConfig,
  options: LLMGenerateOptions = {}
): Promise<{
  headline: TaiyoHeadlineResult;
  body: TaiyoBodyResult;
  bullets: TaiyoBulletResult;
  cta: TaiyoCTAResult;
  ps: TaiyoPSResult;
}> {
  // 並列生成
  const [headline, body, bullets, cta, ps] = await Promise.all([
    generateTaiyoHeadline(config, options),
    generateTaiyoBody(config, options),
    generateTaiyoBullets(config, options),
    generateTaiyoCTA(config, options),
    generateTaiyoPS(config, options),
  ]);

  return { headline, body, bullets, cta, ps };
}
